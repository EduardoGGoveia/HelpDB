<?php
  ...
  if(isset($_GET['idpergunta'])){
    ...
    $result = selecPerg($idpergunta);
    $dados = $result->fetch();
    if(selecAva($dados['id_img']))
    $img = selecAva($dados['id_img']);
  }
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<body>
  ...
  <div class="container-md border border-2 rounded-3 text-center mt-1 py-3" style="max-width: 800px">
  <div class="row d-flex flex-column">
    <div class="col text-start">
      <h5>Pergunta:</h5></br>
    </div>
    <div class="col text-start align-middle">
      <img src="<?php echo $img ?>" width="30" height="30" class="rounded-circle p-0" />
      <label><b><?php echo $dados['nome'] ?></b></label>
    </div>
    <div class="col text-start">
      <label><?php echo $dados['descper']; ?></label>
    </div>
  </div>
  <div class="row d-flex flex-column">
    ...
    <div class="col py-3">
      <textarea class="form-control" style="height: 400px" id="cpResponder" name="cpResponder"></textarea>
    </div>
    <div class="col">
      <a href="" class="btn btn-primary" id="">Responder</a>
    </div>
  </div>
  </div>
  </form>
